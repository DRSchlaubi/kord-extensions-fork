	...
	repositories {
		maven("artifactregistry://europe-west3-maven.pkg.dev/mik-music/mikbot") {
			credentials {
				...
				password = System.getenv("OSSRH_PASSWORD")?.toByteArray()?.let {
...
//                artifact(dokkaJar)
			...
			pom {
				name.set(provider {project.ext.get("pubName").toString() })
				description.set(provider { project.ext.get("pubDesc").toString() })
